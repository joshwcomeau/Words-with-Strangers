...
function rewindAndCaptureWord({ axis: axis, tiles: tiles, gameId: gameId}) {
  ...
  const earliestTile = _.first( _.sortBy(tiles, tile => tile[axis]) );
  ...
  let cursorTile = earliestTile;

  // Now, rewind until we either find an empty square,
  // or we hit the edge of the board.
  while( cursorTile && cursorTile[axis] > 0) {
    let currentPosition = cursorTile[axis];

    cursorTile = Tiles.find({
      gameId: gameId,
      location: 'board',
      position: [cursorTile.x-1, cursorTile.y]
    });
  }
}
