Modules.gameLogic = {
  ...
  submitWord() {
    ...
    // The first step is to invalidate tile sets where they don't form a series
    // of sequential letters on a single axis (horizontal or vertical).
    if ( spansMultipleAxes(activeTiles) ) throw new Error('Not a word');
  }
};
...
function spansMultipleAxes(tiles) {
  // Returns true if the tiles aren't all on the X axis or the Y axis.
  const coordinates = _.pluck(tiles, 'position');

  const num_of_x = _.uniq(_.map(coordinates, coord => coord[0])).length;
  const num_of_y = _.uniq(_.map(coordinates, coord => coord[1])).length;

  return num_of_x > 1 && num_of_y > 1;
}
