...
Modules.gameLogic = {
  ...
  joinGame({gameId, playerId}) {
    // Add the user's ID to the game
    Games.update(gameId, { $push: {
      playerIds: playerId
    }});

    // Give them some tiles!
    this.addTilesToPlayerRack({playerId, gameId, num: Constants.MAX_TILES})
  },
  ...
  submitWord(gameId) {
    ...
    allTilesInWord.forEach( tile => {
      let orthogonalWord = rewindAndCaptureWord({
      ...
      if ( orthogonalWord && orthogonalWord.length > 1 )
        console.log("Found orth word", _.pluck(orthogonalWord, 'letter').join(''));
        orthogonalWords.push(orthogonalWord);
